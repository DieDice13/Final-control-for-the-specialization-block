# Задача организовать систему учета для питомника в котором живут домашние и вьючные животные.

## LINUX команды
1-2. Создание, заполнение и вывод содержимого файлов с помощью команды cat. Обьединеие двух фалйов в один, его переименование и перемещение в другую директорию
```
# Создание и заполнение файла "Домашние животные"
echo "Domestic animals" > Pets.txt
echo "Dogs: Laika, Bobik, Rex" >> Pets.txt
echo "Cats: Murka, Barsik, Vaska" >> Pets.txt
echo "Hamsters: Churchill, Freddy, Hammi" >> Pets.txt
cat Pets.txt

# Создание и заполнение файла "Вьючные животные"
echo "Pack animals" > Pack_animals.txt
echo "Horses: Buran, Spirit, Storm" >> Pack_animals.txt
echo "Camels: Calypso, Gobi, Sahara" >> Pack_animals.txt
echo "Donkeys: Chup, Elliot, Combi" >> Pack_animals.txt
cat Pack_animals.txt

# Объединение содержимого файлов "Домашние животные" и "Вьючные животные" в один файл "Друзья человека"
cat Pets.txt Pack_animals.txt > Human_friends.txt
cat Human_friends.txt

# Создание директории "Animals" и перемещение файла "Друзья человека" в эту директорию
mkdir Animals
mv Human_friends.txt Animals/
```

3. Подключение дополнительного репозитория MySQL. Установка любого пакета из этого репозитория.
```
...
```
# Установка пакета htop с помощью dpkg
sudo dpkg -i htop_3.1.0-1_amd64.deb

# Полное удаление пакета htop, включая его конфигурационные файлы
sudo dpkg -P htop
```
...
```
# Вход в интерактивную оболочку MySQL. 
# Замените 'username' на ваше имя пользователя MySQL.
mysql -u username -p

# Создание базы данных "Друзья человека", если она еще не существует.
...
9. Заполнение низкоуровневых таблиц именами(животных), командами которые они выполняют и датами рождения.
...
10. Удаляем из таблицы, верблюдов т.к. верблюдов решили перевезти вдругой питомник на зимовку. Объединить таблицы лошади,и ослы в одну таблицу.
...
11. Создание новой таблицы “молодые животные” вкоторую попадут все животные старше 1 года, но младше 3 лет и в отдельном столбце с точностью до месяца подсчитываем возраст животных в новой таблице.
